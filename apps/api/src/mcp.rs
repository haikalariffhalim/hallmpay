use hallmpay::{AppConfig, PaymentIntentRequest, TransactionQueryParams};
use mcp::{
...
struct CreatePaymentParams {
    /// Payment channel (1=FPX, 2=Manual, 3=DirectDebit, 5=DuitNow DOBW, 6=DuitNow QR, 12=Credit Card)
    channel: u8,
    /// Order number
    order_number: String,
    /// Payment amount
...
struct VerifyCallbackParams {
    ...
    secret_key: String,
    /// Callback type: transaction, pre_transaction, return_url, directdebit_approval, directdebit_authorization, directdebit_transaction
...
impl HallmPay {
    ...
    #[tool(
        description = "Create a HallmPay payment intent. Returns the payment URL and intent details."
    )]
    ...
    #[tool(description = "Get a payment intent by ID.")]
    ...
    #[tool(
        description = "List transactions with optional filters (order_number, status, payment_channel, payer_email, exchange_reference_number)."
    )]
    ...
    #[tool(description = "Generate HMAC-SHA256 checksum for a payment intent request.")]
    ...
    #[tool(
        description = "Verify a HallmPay callback signature. callback_type must be one of: transaction, pre_transaction, return_url, directdebit_approval, directdebit_authorization, directdebit_transaction"
    )]
    async fn verify_callback(&self, #[tool(aggr)] params: VerifyCallbackParams) -> String {
        let result = match params.callback_type.as_str() {
            "transaction" => {
                match serde_json::from_value::<hallmpay::TransactionCallbackData>(
                    params.callback_data,
                ) {
                    Ok(data) => hallmpay::verification::verify_transaction(&data, &params.secret_key),
                    Err(e) => return format!("Failed to parse callback data: {}", e),
                }
            }
            "pre_transaction" => {
                match serde_json::from_value::<hallmpay::PreTransactionCallbackData>(
                    params.callback_data,
                ) {
                    Ok(data) => {
                        hallmpay::verification::verify_pre_transaction(&data, &params.secret_key)
                    }
                    Err(e) => return format!("Failed to parse callback data: {}", e),
                }
            }
            "return_url" => {
                match serde_json::from_value::<hallmpay::ReturnUrlCallbackData>(
                    params.callback_data,
                ) {
                    Ok(data) => {
                        hallmpay::verification::verify_return_url(&data, &params.secret_key)
                    }
                    Err(e) => return format!("Failed to parse callback data: {}", e),
                }
            }
            "directdebit_approval" => {
                match serde_json::from_value::<hallmpay::DirectDebitBankApprovalCallbackData>(
                    params.callback_data,
                ) {
                    Ok(data) => hallmpay::verification::verify_direct_debit_bank_approval(
                        &data,
                        &params.secret_key,
                    ),
                    Err(e) => return format!("Failed to parse callback data: {}", e),
            ...
            "directdebit_authorization" => {
                match serde_json::from_value::<hallmpay::DirectDebitAuthorizationCallbackData>(
                    params.callback_data,
                ) {
                    Ok(data) => hallmpay::verification::verify_direct_debit_authorization(
                        &data,
                        &params.secret_key,
                    ),
                    Err(e) => return format!("Failed to parse callback data: {}", e),
            ...
            "directdebit_transaction" => {
                match serde_json::from_value::<hallmpay::DirectDebitTransactionCallbackData>(
                    params.callback_data,
                ) {
                    Ok(data) => hallmpay::verification::verify_direct_debit_transaction(
                        &data,
                        &params.secret_key,
                    ),
                    Err(e) => return format!("Failed to parse callback data: {}", e),
            ...
            other => {
                return format!(
                    "Unknown callback type '{}'. Use: transaction, pre_transaction, return_url, directdebit_approval, directdebit_authorization, directdebit_transaction",
                    other
                )
            }
        };
    ...
    async fn get_mandate(&self, #[tool(aggr)] params: IdParam) -> String {
        ...
        match client.get_fpx_direct_debit(&params.id).await {
            Ok(directdebit) => serde_json::to_string_pretty(&directdebit)
                .unwrap_or_else(|e| format!("Serialization error: {}", e)),
            Err(e) => format!("Error: {}", e),
        }
    }
...
impl ServerHandler for HallmPay {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            server_info: Implementation {
                name: "HallmPayMCP".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
            },
            instructions: Some(
				                "HallmPayMCP server for managing payments, transactions, portals, and FPX Direct Debit mandates. \
				                Set HALLMPAY_TOKEN env var (and optionally HALLMPAY_SECRET_KEY, HALLMPAY_SANDBOX=true, HALLMPAY_API_VERSION=v1)."
                    .to_string(),
            ),
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = HallmPay::new(config);
