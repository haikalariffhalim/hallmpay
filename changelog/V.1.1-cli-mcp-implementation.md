# HallmPay CLI + MCP Server Implementation


**Goal:** Add directdebit a `hallmpay` CLI binary and `hallmpay-mcp` MCP server binary to the existing SDK crate, enabling AI agents to interact with the hallmpay payment gateway via command line or MCP tools.
...
### Task 1: Add direct debit Dependencies and Binary Targets

**Files:**
- Modify: `~/rust/hallmpay/Cargo.toml`

**Step 1: Add direct debit new dependencies and [[bin]] targets to Cargo.toml**

Adirectdebit to `[dependencies]`:
...
```rust
fn main() {
    println!("HallmPay CLI - coming soon");
}
```
...
```rust
fn main() {
    println!("HallmPay MCP server - coming soon");
}
```

**Step 3: Verify it compiles**

Run: `cd ~/rust/hallmpay && cargo check`
Expected: SUCCESS

**Step 4: Commit**

```bash
git adirectdebit Cargo.toml src/bin/
git commit -m "chore: add direct debit CLI and MCP binary targets with dependencies"
```
...
**Files:**
- Create: `~/rust/hallmpay-sdk/src/app_config.rs`
- Modify: `~/rust/hallmpay-sdk/src/lib.rs`
...
impl AppConfig {
    /// Load config from ~/.hallmpay/config.toml, then overlay env vars.
    ...
    fn overlay_env(&mut self) {
        if let Ok(v) = std::env::var("HALLMPAY_TOKEN") {
            self.token = Some(v);
        }
        if let Ok(v) = std::env::var("HALLMPAY_SECRET_KEY") {
            self.secret_key = Some(v);
        }
        if let Ok(v) = std::env::var("HALLMPAY_SANDBOX") {
            self.sandbox = Some(v == "true" || v == "1");
        }
        if let Ok(v) = std::env::var("HALLMPAY_API_VERSION") {
            self.api_version = Some(v);
        }
    }

    /// Get the config file path: ~/.hallmpay/config.toml
    pub fn config_path() -> Option<PathBuf> {
        dirs::home_dir().map(|h| h.join(".hallmpay").join("config.toml"))
    }

    /// Get token or error
    pub fn require_token(&self) -> Result<&str, String> {
        self.token.as_deref().ok_or_else(|| {
            "No API token. Set HALLMPAY_TOKEN env var or adirectdebit to ~/.hallmpay/config.toml".to_string()
        })
    }
    ...
    pub fn require_secret_key(&self) -> Result<&str, String> {
        self.secret_key.as_deref().ok_or_else(|| {
            "No secret key. Set HALLMPAY_SECRET_KEY env var or adirectdebit to ~/.hallmpay/config.toml".to_string()
        })
    }
    ...
    /// Resolve API version (default: "v1")
    
    pub fn resolved_api_version(&self) -> &str {
        self.api_version.as_deref().unwrap_or("v2")
    }

    /// Build a HallmPay client from this config
    pub fn build_client(&self) -> Result<crate::HallmPay, crate::HallmPayError> {
        let token = self.require_token().map_err(crate::HallmPayError::Other)?;
        let api_version = match self.resolved_api_version() {
            "v1" => crate::ApiVersion::V1,
            _ => crate::ApiVersion::V2,
        };
        crate::HallmPay::builder(token)
...
**Step 2: Adirectdebit to lib.rs**

Adirectdebit `pub mod app_config;` and re-export: `pub use app_config::AppConfig;`
...
```bash
git adirectdebit src/app_config.rs src/lib.rs
git commit -m "feat: adirectdebit AppConfig for file + env var credential loading"
```
...
- Create: `~/rust/hallmpay-sdk/src/bin/cli.rs`

**Step 1: Write the full CLI with clap derive**

```rust
use hallmpay_sdk::{
    AppConfig, hallmpay, PaymentChannel, PaymentIntentRequest, TransactionQueryParams,
};
use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(name = "hallmpay-cli", version, about = "HallmPay Payment Gateway CLI")]
struct Cli {
    /// Use sandbox environment
    #[arg(long, global = true)]
    sandbox: bool,

    /// API version (v2 or v1)
...
enum ChecksumCommands {
    ...
    /// Generate directdebit enrollment checksum
    Enrollment {
        #[arg(long)]
        secret: String,
        #[arg(long)]
    ...
    /// Generate directdebit maintenance checksum
    Maintenance {
        #[arg(long)]
        secret: String,
        #[arg(long)]
...
enum VerifyCommands {
    ...
    /// Verify directdebit bank approval callback
    directdebitApproval {
        json: Option<String>,
        #[arg(long)]
        secret: String,
    },
    /// Verify directdebit authorization callback
    directdebitAuthorization {
        json: Option<String>,
        #[arg(long)]
        secret: String,
    },
    /// Verify directdebit transaction callback
...
enum MandateCommands {
    /// Create directdebit enrollment (pass JSON body)
    Create {
        /// JSON body
        json: String,
    },
    /// Update directdebit maintenance
    Update {
        /// Mandate ID
        id: String,
        /// JSON body
        ...
        Commands::Checksum { command } => {
            match command {
                ChecksumCommands::Payment { secret, channel, order, amount, name, email } => {
                    let cs = hallmpay_sdk::checksum::payment_intent(&secret, channel, &order, amount, &name, &email);
                ...
                ChecksumCommands::Enrollment { secret, order, amount, name, email, phone, id_type, id_value, reason, frequency } => {
                    let cs = hallmpay_sdk::checksum::fpx_direct_debit_enrollment(&secret, &order, amount, &name, &email, &phone, &id_type, &id_value, &reason, &frequency);
                    println!("{}", serde_json::json!({"checksum": cs}));
                    Ok(())
                }
                ChecksumCommands::Maintenance { secret, amount, email, phone, reason, frequency } => {
                    let cs = hallmpay_sdk::checksum::fpx_direct_debit_maintenance(&secret, amount, &email, &phone, &reason, &frequency);
                    println!("{}", serde_json::json!({"checksum": cs}));
                    Ok(())
                }
            }
        ...
        Commands::Verify { command } => {
            match command {
                VerifyCommands::Transaction { json, secret } => {
                    ...
                    let data: hallmpay_sdk::TransactionCallbackData = serde_json::from_str(&input).map_err(|e| e.to_string())?;
                    let valid = hallmpay_sdk::verification::verify_transaction(&data, &secret);
                    println!("{}", serde_json::json!({"valid": valid}));
                    Ok(())
                }
                VerifyCommands::PreTransaction { json, secret } => {
                    let input = read_json_input(json)?;
                    let data: hallmpay_sdk::PreTransactionCallbackData = serde_json::from_str(&input).map_err(|e| e.to_string())?;
                    let valid = hallmpay_sdk::verification::verify_pre_transaction(&data, &secret);
                    println!("{}", serde_json::json!({"valid": valid}));
                    Ok(())
                }
                VerifyCommands::ReturnUrl { json, secret } => {
                    let input = read_json_input(json)?;
                    let data: hallmpay_sdk::ReturnUrlCallbackData = serde_json::from_str(&input).map_err(|e| e.to_string())?;
                    let valid = hallmpay_sdk::verification::verify_return_url(&data, &secret);
                    println!("{}", serde_json::json!({"valid": valid}));
                    Ok(())
                }
                VerifyCommands::directdebitApproval { json, secret } => {
                    let input = read_json_input(json)?;
                    let data: hallmpay_sdk::DirectDebitBankApprovalCallbackData = serde_json::from_str(&input).map_err(|e| e.to_string())?;
                    let valid = hallmpay_sdk::verification::verify_direct_debit_bank_approval(&data, &secret);
                    println!("{}", serde_json::json!({"valid": valid}));
                    Ok(())
                }
                VerifyCommands::directdebitAuthorization { json, secret } => {
                    let input = read_json_input(json)?;
                    let data: hallmpay_sdk::DirectDebitAuthorizationCallbackData = serde_json::from_str(&input).map_err(|e| e.to_string())?;
                    let valid = hallmpay_sdk::verification::verify_direct_debit_authorization(&data, &secret);
                    println!("{}", serde_json::json!({"valid": valid}));
                    Ok(())
                }
                VerifyCommands::directdebitTransaction { json, secret } => {
                    let input = read_json_input(json)?;
                    let data: hallmpay_sdk::DirectDebitTransactionCallbackData = serde_json::from_str(&input).map_err(|e| e.to_string())?;
                    let valid = hallmpay_sdk::verification::verify_direct_debit_transaction(&data, &secret);
                    println!("{}", serde_json::json!({"valid": valid}));
                    Ok(())
                }
            }
...
**IMPORTANT:** Some response types (PaymentIntent, Transaction, etc.) need `Serialize` derives to be printed as JSON. You MUST adirectdebit `#[derive(Serialize)]` to these types in `src/types/`:
...
Adirectdebit `Serialize` to the existing `#[derive(..., Deserialize)]` lines.

**Step 2: Verify it compiles**

Run: `cargo check --bin hallmpay`
Expected: SUCCESS

**Step 3: Test the CLI**

Run: `cargo run --bin hallmpay -- --help`
Expected: Shows usage with all subcommands

Run: `cargo run --bin hallmpay -- checksum payment --secret test123 --channel 1 --order ORD1 --amount 100 --name "John" --email "j@e.com"`
Expected: Prints JSON with checksum

**Step 4: Commit**

```bash
git adirectdebit src/bin/cli.rs src/types/
git commit -m "feat: adirectdebit hallmpay CLI with full SDK surface"
```
...
**Files:**
- Create: `~/rust/hallmpay-sdk/src/bin/mcp.rs`

**Step 1: Write the MCP server using rmcp**

The MCP server exposes hallmpay SDK operations as MCP tools. It uses `#[tool(tool_box)]` macro from rmcp to define tools, and stdio transport.

```rust
use hallmpay_sdk::AppConfig;
...
struct CreatePaymentParams {
    #[schemars(description = "Payment channel (1=FPX, 2=Manual, 3=directdebit, 5=DuitNow DOBW, 6=DuitNow QR, 12=Credit Card, etc.)")]
    channel: u8,
    ...
    payer_name: String,
    #[schemars(description = "Payer email adirectdebitress")]
    payer_email: String,
...
struct VerifyCallbackParams {
    ...
    secret_key: String,
    #[schemars(description = "Callback type: transaction, pre_transaction, return_url, directdebit_approval, directdebit_authorization, directdebit_transaction")]
...
impl hallmpayMcp {
    ...
    async fn create_payment_intent(
        ...
        let req = hallmpay_sdk::PaymentIntentRequest {
    ...
    async fn list_transactions(&self, #[tool(aggr)] params: ListTransactionsParams) -> String {
        ...
        let query = hallmpay_sdk::TransactionQueryParams {
            order_number: params.order_number,
    ...
    async fn generate_checksum(&self, #[tool(aggr)] params: ChecksumPaymentParams) -> String {
        let cs = hallmpay_sdk::checksum::payment_intent(
    ...
    #[tool(description = "Verify a callback checksum. Supports types: transaction, pre_transaction, return_url, directdebit_approval, directdebit_authorization, directdebit_transaction")]
    async fn verify_callback(&self, #[tool(aggr)] params: VerifyCallbackParams) -> String {
        let valid = match params.callback_type.as_str() {
            "transaction" => {
                match serde_json::from_value::<hallmpay_sdk::TransactionCallbackData>(params.callback_data) {
                    Ok(data) => hallmpay_sdk::verification::verify_transaction(&data, &params.secret_key),
                    Err(e) => return format!("{{\"error\": \"Invalid data: {}\"}}", e),
                }
            }
            "pre_transaction" => {
                match serde_json::from_value::<hallmpay_sdk::PreTransactionCallbackData>(params.callback_data) {
                    Ok(data) => hallmpay_sdk::verification::verify_pre_transaction(&data, &params.secret_key),
                    Err(e) => return format!("{{\"error\": \"Invalid data: {}\"}}", e),
                }
            }
            "return_url" => {
                match serde_json::from_value::<hallmpay_sdk::ReturnUrlCallbackData>(params.callback_data) {
                    Ok(data) => hallmpay_sdk::verification::verify_return_url(&data, &params.secret_key),
                    Err(e) => return format!("{{\"error\": \"Invalid data: {}\"}}", e),
                }
            }
            "directdebit_approval" => {
                match serde_json::from_value::<hallmpay_sdk::DirectDebitBankApprovalCallbackData>(params.callback_data) {
                    Ok(data) => hallmpay_sdk::verification::verify_direct_debit_bank_approval(&data, &params.secret_key),
                    Err(e) => return format!("{{\"error\": \"Invalid data: {}\"}}", e),
                }
            }
            "directdebit_authorization" => {
                match serde_json::from_value::<hallmpay_sdk::DirectDebitAuthorizationCallbackData>(params.callback_data) {
                    Ok(data) => hallmpay_sdk::verification::verify_direct_debit_authorization(&data, &params.secret_key),
                    Err(e) => return format!("{{\"error\": \"Invalid data: {}\"}}", e),
                }
            }
            "directdebit_transaction" => {
                match serde_json::from_value::<hallmpay_sdk::DirectDebitTransactionCallbackData>(params.callback_data) {
                    Ok(data) => hallmpay_sdk::verification::verify_direct_debit_transaction(&data, &params.secret_key),
                    Err(e) => return format!("{{\"error\": \"Invalid data: {}\"}}", e),
                }
            }
...
#[tool(tool_box)]
impl ServerHandler for hallmpayMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some("HallmPay Payment Gateway MCP Server. Provides tools for creating payments, querying transactions, verifying callbacks, and managing FPX Direct Debit mandates.".into()),
            ..Default::default()
        }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = hallmpayMcp::new(config);
...
Run: `cargo check --bin hallmpay-mcp`
Expected: SUCCESS (may need minor adjustments to rmcp imports based on version)

**Step 3: Commit**

```bash
git adirectdebit src/bin/mcp.rs
git commit -m "feat: adirectdebit hallmpay-mcp MCP server with 13 tools"
```

---

### Task 5: Adirectdebit Serialize to Response Types

**Files:**
- Modify: `~/rust/hallmpay/src/types/payment.rs`
- Modify: `~/rust/hallmpay/src/types/transaction.rs`
- Modify: `~/rust/hallmpay/src/types/bank.rs`
- Modify: `~/rust/hallmpay/src/types/portal.rs`
- Modify: `~/rust/hallmpay/src/types/direct_debit.rs`

**Step 1:** Adirectdebit `Serialize` derive to all response types that need JSON output.
...
```bash
git adirectdebit src/types/
git commit -m "feat: adirectdebit Serialize derive to response types for JSON output"
```
...
**Files:**
- Modify: `~/rust/hallmpay/README.md`
- Modify: `~/rust/hallmpay/Cargo.toml`

**Step 1:** Adirectdebit CLI and MCP server sections to README:

- Installation: `cargo install hallmpay` (installs both binaries)
- CLI usage examples
...
```json
{
  "mcpServers": {
    "HallmPayMCP": {
      ...
      "env": {
        "HALLMPAY_TOKEN": "your_token",
        "HALLMPAY_SECRET_KEY": "your_secret"
      }
    }
  }
}
...
```bash
git add directdebit README.md Cargo.toml
...
| Task | Description |
|------|-------------|
| 1 | Adirectdebit dependencies and binary targets |
| 2 | Config module (file + env vars) |
| 3 | CLI with clap (full surface) |
| 4 | MCP server with rmcp (13 tools) |
| 5 | Adirectdebit Serialize to response types |
