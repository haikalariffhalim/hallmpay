use crate::config::HallmPayConfig;
use crate::error::{HallmPayError, Result};
use crate::http::HttpClient;
use crate::types::*;
use std::time::Duration;

/// HallmPay  payment gateway client
pub struct HallmPay{
    pub(crate) config: HallmPayConfig,
    pub(crate) http: HttpClient,
}

/// Builder for constructing a HallmPay  client
pub struct HallmPayBuilder {
    token: String,
    sandbox: bool,
...
impl HallmPay {
    ...
    pub fn builder(token: &str) -> HallmPayBuilder {
        HallmPayBuilder::builder(token)
    }

    /// Helper to enforce version-specific features
    fn require_v2(&self, method_name: &str) -> Result<()> {
        if self.config.api_version != ApiVersion::V2 {
            return Err(HallmPayError::ApiVersionMismatch(method_name.to_string()));
        }
        Ok(())
    }
    ...
    pub async fn get_payment_intent(&self, id: &str) -> Result<PaymentIntent> {
        self.require_v2("get_payment_intent")?;
    ...
    pub async fn get_portals(&self) -> Result<Vec<Portal>> {
        ...
        if raw.is_array() {
            return serde_json::from_value(raw).map_err(|e| HallmPayError::Other(e.to_string()));
        }
        if let Some(data) = raw.get("data") {
            return serde_json::from_value(data.clone())
                .map_err(|e| HallmPayError::Other(e.to_string()));
        }
        Ok(vec![])
    }
    ...
    pub async fn get_all_transactions(
        ...
        self.require_v2("get_all_transactions")?;
    ...
    pub async fn get_transactions_by_order(&self, order_number: &str) -> Result<Vec<Transaction>> {
        self.require_v2("get_transactions_by_order")?;
    ...
    pub async fn get_transactions_by_email(&self, email: &str) -> Result<Vec<Transaction>> {
        self.require_v2("get_transactions_by_email")?;
        let resp: PaginatedResponse<Transaction> = self
            .http
            .get(&format!(
    ...
    pub async fn get_transactions_by_status(&self, status: &str) -> Result<Vec<Transaction>> {
        self.require_v2("get_transactions_by_status")?;
        let resp: PaginatedResponse<Transaction> = self
            .http
            .get(&format!(
                "transactions?status={}",
    ...
    pub async fn get_transactions_by_channel(
        ...
        self.require_v2("get_transactions_by_channel")?;
        let resp: PaginatedResponse<Transaction> = self
            .http
    ...
    pub async fn get_transaction_by_reference(
        ...
        self.require_v2("get_transaction_by_reference")?;
        let resp: PaginatedResponse<Transaction> = self
            .http
            .get(&format!(
    ...
    pub async fn create_manual_bank_transfer(
        ...
        if data.payment_gateway != 2 {
            return Err(HallmPayError::Other(
                "payment gateway must be 2 way for manual transfers".to_string(),
            ));
        }
        let url = format!(
            "{}/manual-bank-transfer",
        ...
        if let Some(ref path) = data.proof_of_payment_path {
            let file_bytes = tokio::fs::read(path)
                .await
                .map_err(|e| HallmPayError::Other(format!("Failed to read file: {}", e)))?;
...
impl HallmPayBuilder {
    ...
    pub fn build(self) -> Result<HallmPay> {
        let config = HallmPayConfig {
        ...
        Ok(HallmPay { config, http })
    }

    /// Build with a custom base URL (for testing with mockito)
    pub fn build_with_base_url(self, base_url: &str) -> Result<HallmPay> {
        let config = HallmPayConfig {
            token: self.token.clone(),
            sandbox: self.sandbox,
            api_version: self.api_version,
