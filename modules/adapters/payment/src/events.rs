...
use crate::error::PayNetAdapterError;
use metering_grpc::HallmPay::metering::v1 as server;
use metering_grpc::HallmPay::metering::v1::IngestRequest;

pub struct EventHandler {
    pub sink: HallmPaySink,
    pub source: RabbitSource,
    pub config: Config,
}

impl EventHandler {
    pub async fn start(&mut self) -> Result<(), PayNetAdapterError> {
        let conn = &self.source.connection;

        let event_channel = conn
            .create_channel()
            .await
            .map_err(PayNetAdapterError::LapinError)?;
    ...
    async fn consume_events(
        ...
        channel: Channel,
        queue: &str,
    ) -> Result<(), PayNetAdapterError> {
        let consumer = channel
            .basic_consume(
                queue.into(),
                "openstack_HallmPay_consumer".into(),
    ...
    async fn handle_messages(&mut self, mut consumer: Consumer) -> Result<(), PayNetAdapterError> {
        while let Some(delivery) = consumer.next().await {
            let delivery = delivery.map_err(PayNetAdapterError::LapinError)?;

            let oslo_event: OsloRecord = serde_json::from_slice(&delivery.data).map_err(|e| {
                PayNetAdapterError::SerializationError(
            ...
            let events: Vec<server::Event> = match event.event_type {
                CeilometerEventType::Metering => {
                    ...
                    payloads
                        ...
                        .collect::<Result<Vec<Option<server::Event>>, PayNetAdapterError>>()?
                        .into_iter()
                        .flatten()
                        .collect()
                }
            ...
            if !events.is_empty() {
                let res = self
                    ...
                    .await
                    .map_err(PayNetAdapterError::GrpcError)?;
    ...
    ) -> Result<Option<server::Event>, PayNetAdapterError> {
        ...
        match event.event_type.as_str() {
            ...
            | "compute.instance.resize.confirm.end" => {
                let project_id = event
                    ...
                    .ok_or_else(|| {
                        PayNetAdapterError::HandlerError(format!(
                ...
                let instance_id = event
                    ...
                    .and_then(|x| x.value.as_str())
                    .ok_or_else(|| {
                        PayNetAdapterError::HandlerError("Failed to decode Instance ID".to_string())
                    })?;
                let flavor = event
                    .traits
                    .iter()
                    ...
                    .and_then(|x| x.value.as_str())
                    .ok_or_else(|| {
                        PayNetAdapterError::HandlerError(
