use crate::error::{ErrorResponse, PayNetError};
...
pub type Response<T> = Pin<Box<dyn Future<Output = Result<T, PayNetError>> + Send>>;

static USER_AGENT: &str = concat!(
    "HallmPay/PayNet/v1 RustBindings/",
...
#[derive(Debug, Clone)]
pub struct PayNetHeaders {
    pub paynet_version: String,
    pub user_agent: String,

    pub client_id: Option<String>,
    pub paynet_account: Option<String>,
}
...
impl PayNetHeaders {
    pub fn as_header_map(&self) -> HeaderMap {
        ...
        if let Some(paynet_account) = &self.paynet_account {
            add_header("PayNet-Account", paynet_account);
        }
        add_header("PayNet-Version", &self.paynet_version);
...
impl PayNetClient {
    ...
    pub fn from_parts<'a>(
        ...
        Self {
            ...
            headers: PayNetHeaders {
                paynet_version: API_VERSION.to_string(),
    ...
    pub(crate) fn post_form<T: DeserializeOwned + Send + 'static, F: Serialize>(
        ...
        if let Err(qs_ser_err) = serde_path_to_error::serialize(&form, qs_ser) {
            return self.err(PayNetError::QueryStringSerialize(qs_ser_err));
        }
    ...
    #[allow(dead_code)]
    #[inline(always)]
    fn err<T: Send + 'static>(&self, err: PayNetError) -> Response<T> {
    ...
    pub fn execute<T: DeserializeOwned + Send>(
        ...
        Box::pin(async move {
            ...
            serde_path_to_error::deserialize(json_deserializer).map_err(PayNetError::from)
        })
    }
    ...
    async fn send_inner(
        ...
        retry_strategy: RetryStrategy,
    ) -> Result<Bytes, PayNetError> {
        let mut tries: u32 = 0;

        loop {
            let response = req_builder
                .try_clone()
                .ok_or(PayNetError::ClientError(
            ...
            match response {
                Ok(resp) => {
                    ...
                    if resp_status.is_success() {
                        ...
                        let paynet_retry = resp_headers
                            .get("PayNet-Should-Retry")
                            ...
                            Outcome::Stop => {
                                ...
                                let error = serde_path_to_error::deserialize(json_deserializer)
                                    .map(|mut e: ErrorResponse| {
                                        ...
                                        PayNetError::from(e.error)
                                    })
                                    .unwrap_or_else(PayNetError::from);
                ...
                Err(err) => match retry_strategy.test(None, None, tries) {
                    Outcome::Stop => return Err(PayNetError::from(err)),
                    Outcome::Continue(sleep_duration) => {
                        tries += 1;
                        tokio::time::sleep(sleep_duration).await;
                        continue;
...
#[cfg(test)]
mod tests {
    use crate::client::{PayNetHeaders, API_VERSION, USER_AGENT};
    use reqwest::header::HeaderValue;

    #[test]
    fn test_paynet_headers() {
        let strategy = PayNetHeaders {
            paynet_version: API_VERSION.to_string(),
            user_agent: USER_AGENT.to_string(),
            client_id: None,
            paynet_account: None,
        ...
        assert_eq!(header_map.get("PayNet-Account"), None);
    }
}
