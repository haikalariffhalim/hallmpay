...
use paynet_client::webhook::Event;
use paynet_client::webhook::EventObject;
...
use hallmpay_store::domain::{CustomerPaymentMethodNew, PaymentIntent};
use hallmpay_store::repositories::customer_payment_methods::CustomerPaymentMethodsInterface;
use hallmpay_store::Store;
use paynet_client::setup_intents::SetupIntent;
use paynet_client::webhook::event_type;
use paynet_client::webhook::paynetWebhook;

use hallmpay_store::adapters::payment_service_providers::{PaymentProvider, PaymentProviderError};
use hallmpay_store::domain::connectors::Connector;
use hallmpay_store::repositories::payment_transactions::PaymentTransactionInterface;
use hallmpay_store::repositories::CustomersInterface;

static paynet: std::sync::OnceLock<PayNet> = std::sync::OnceLock::new();

#[derive(Debug, Clone)]
pub struct PayNet {
    pub client: Arc<paynet_client::client::PayNetClient>,
}

impl AdapterCommon for PayNet {
...
impl WebhookAdapter for PayNet {
    async fn verify_webhook(
        ...
        PayNetWebhook::validate_signature(
    ...
    async fn process_webhook_event(
        ...
        let parsed = PayNetWebhook::parse_event(request.json_body.to_string().as_str())
...
impl PayNet {
    pub fn get() -> &'static Self {
        paynet.get_or_init(|| PayNet {
            client: Arc::new(paynet_client::client::paynetClient::new()),
        })
    }
    ...
    async fn process_setup_intent_events(
        ...
        let connection_id = data.metadata.get("hallmpay.connection_id").ok_or(
            errors::AdapterWebhookError::MissingMetadata("hallmpay.connection_id".to_string()),
        )?;
        ...
        let customer_id = data.metadata.get("hallmpay.customer_id").ok_or(
            errors::AdapterWebhookError::MissingMetadata("hallmpay.customer_id".to_string()),
        )?;
        ...
        use hallmpay_store::domain::CustomerPatch;
