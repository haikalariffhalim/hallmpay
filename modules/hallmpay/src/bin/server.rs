...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let paynet = Arc::new(paynetClient::new());
    ...
    let grpc_server = hallmpay::api::server::start_api_server(
        config.clone(),
        store.clone(),
        services.clone(),
    ...
    let paynet = Arc::new(PayNetClient::new());
    let paynet_adapter = Arc::new(PayNet { client: paynet });

    let rest_server = hallmpay::api_rest::server::start_rest_server(
        config.clone(),
        object_store_service.clone(),
        paynet_adapter.clone(),
        store.clone(),
        services.clone(),
        ready.clone(),
