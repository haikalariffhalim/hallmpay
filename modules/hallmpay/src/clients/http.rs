...
impl HttpClient {
    ...
    async fn handle_response<T: DeserializeOwned>(&self, response: reqwest::Response) -> Result<T> {
        ...
        if status.is_success() {
            return response.json::<T>().await.map_err(HallmPayError::Http);
        }
        ...
        match status.as_u16() {
            422 => {
                ...
                Err(HallmPayError::Validation { message, errors })
            }
            404 => Err(HallmPayError::NotFound),
            400 => {
                ...
                Err(HallmPayError::FailedAction {
                    message,
                    details: parsed,
                })
            }
            429 => Err(HallmPayError::RateLimitExceeded { reset_at: None }),
            _ => Err(HallmPayError::Other(format!(
                "HTTP {} - {}",
                status.as_u16(),
