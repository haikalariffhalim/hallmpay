...
#[derive(Parser)]
#[command(name = "hallmpay_cli ",  about = "HallmPay Payment CLI")]
struct Cli {
...
enum PaymentCommands {
    ...
    /// Get a payment intent by ID (v2 only)
    Get {
        /// Payment intent ID
        id: String,
    },
...
enum TransactionCommands {
    ...
    /// List transactions with filters (v2 only)
    List {
        #[arg(long)]
        order: Option<String>,
        #[arg(long)]
    ...
    match command {
        ...
        Commands::Checksum { command } => match command {
            ChecksumCommands::Payment {
                ...
                let cs = hallmpay::checksum::payment_intent(
            ...
            ChecksumCommands::Enrollment {
                ...
                let cs = hallmpay::checksum::fpx_direct_debit_enrollment(
            ...
            ChecksumCommands::Maintenance {
                ...
                let cs = hallmpay::checksum::fpx_direct_debit_maintenance(
        ...
        Commands::Verify { command } => match command {
            VerifyCommands::Transaction { json, secret } => {
                ...
                let data: hallmpay::TransactionCallbackData =
                    serde_json::from_str(&input).map_err(|e| e.to_string())?;
                let valid = hallmpay::verification::verify_transaction(&data, &secret);
                println!("{}", serde_json::json!({"valid": valid}));
                Ok(())
            }
            VerifyCommands::PreTransaction { json, secret } => {
                let input = read_json_input(json)?;
                let data: hallmpay::PreTransactionCallbackData =
                    serde_json::from_str(&input).map_err(|e| e.to_string())?;
                let valid = hallmpay::verification::verify_pre_transaction(&data, &secret);
                println!("{}", serde_json::json!({"valid": valid}));
                Ok(())
            }
            VerifyCommands::ReturnUrl { json, secret } => {
                let input = read_json_input(json)?;
                let data: hallmpay::ReturnUrlCallbackData =
                    serde_json::from_str(&input).map_err(|e| e.to_string())?;
                let valid = hallmpay::verification::verify_return_url(&data, &secret);
                println!("{}", serde_json::json!({"valid": valid}));
                Ok(())
            }
            VerifyCommands::DirectDebitApproval { json, secret } => {
                let input = read_json_input(json)?;
                let data: hallmpay::DirectDebitBankApprovalCallbackData =
                    serde_json::from_str(&input).map_err(|e| e.to_string())?;
                let valid =
                    hallmpay::verification::verify_direct_debit_bank_approval(&data, &secret);
                println!("{}", serde_json::json!({"valid": valid}));
                Ok(())
            }
            VerifyCommands::DirectDebitAuthorization { json, secret } => {
                let input = read_json_input(json)?;
                let data: hallmpay::DirectDebitAuthorizationCallbackData =
                    serde_json::from_str(&input).map_err(|e| e.to_string())?;
                let valid =
                    hallmpay::verification::verify_direct_debit_authorization(&data, &secret);
                println!("{}", serde_json::json!({"valid": valid}));
                Ok(())
            }
            VerifyCommands::DirectDebitTransaction { json, secret } => {
                let input = read_json_input(json)?;
                let data: hallmpay::DirectDebitTransactionCallbackData =
                    serde_json::from_str(&input).map_err(|e| e.to_string())?;
                let valid =
                    hallmpay::verification::verify_direct_debit_transaction(&data, &secret);
                println!("{}", serde_json::json!({"valid": valid}));
                Ok(())
            }
        },
